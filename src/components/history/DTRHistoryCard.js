...
import { CommonCardStyle } from "./DTRHistoryCardStyle";
...
export default function DTRHistoryCard({ data, imageCenterPath, imagePressed, approvePress, returnPress, cardPress, }) {
            ...
            <TouchableOpacity style={CommonCardStyle.card} onPress={cardPress}>
                ...
                <View style={CommonCardStyle.cardContainer}>
                    <View style={{ flexDirection: 'column' }}>
                        ...
                        {data?.geotag_list?.status == "Return" && <Text style={CommonCardStyle.status1}>{data?.geotag_list?.status}</Text>}
                        {data?.geotag_list?.status == "Returned" && <Text style={CommonCardStyle.status1}>{data?.geotag_list?.status}</Text>}
                        {data?.geotag_list?.status == "Approved" && <Text style={CommonCardStyle.status2}>{data?.geotag_list?.status}</Text>}
                    </View>
