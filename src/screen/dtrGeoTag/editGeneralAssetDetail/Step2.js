...
const Step2 = ({ handleButtonPress, data, item, pacageList }) => {
  ...
  useEffect(() => {
    // const filtered = data.dtr_capacity.filter(obj => {
    //   return obj.title === item.dtr_capacity;
    // });
    // setDtrCapacity(filtered);
    ...
    setManufacturingDate(new Date(item.dtr_details.dtr_manufacturing_month_year));
    setInstallationDate(new Date(item.dtr_details.dtr_year_installation));
  ...
  const onSubmit = () => {
    ...
    handleButtonPress({
      ...
      dtr_capacity: isDtrCapacity?.id ? isDtrCapacity?.id : isDtrCapacity,
      dtr_voltage_ratio: isDtrVoltage?.id ? isDtrVoltage?.id : isDtrVoltage,
      dtr_model: isMake,
      dtr_manufacturing_month_year: isManufacturingDate,
      dtr_year_installation: isInstallationDate,
      dtr_mounting: isDtrMounting?.id ? isDtrMounting?.id : isDtrMounting,
  ...
  return (
    <>
      ...
      <View style={GeneralAssetDetailStyle.marginTop}>
        <SingleValueDropdownListSetOldData oldIndex={""} title={"Package No."} oldIndex={pacageList?.indexOf(item?.dtr_details?.dtr_Package)} data={pacageList} onSelectedSevice={(value) => setPackege(value)} />
      ...
      <View style={GeneralAssetDetailStyle.marginTop}>
        <SingleDropdownList title={"DTR Capacity (KVA)*"} defoultVaule={item.dtr_details.dtr_capacity} data={data?.dtr_capacity} onSelectedSevice={(selectState) => setDtrCapacity(selectState)} />
        <Text style={GeneralAssetDetailStyle.asaText}>*(6/15/25/…)</Text>
      </View>

      <View style={GeneralAssetDetailStyle.marginTop}>
        <SingleDropdownList title={"DTR Voltage Ratio"} defoultVaule={item.dtr_details.dtr_voltage_ratio} data={data?.dtr_voltage_ratio} onSelectedSevice={(selectState) => setDtrVoltage(selectState)} />
      </View>
      ...
      <View style={GeneralAssetDetailStyle.marginTop}>
        <SingleDropdownList title={"DTR Mounting*"} defoultVaule={item.dtr_details.dtr_mounting} data={data?.dtr_mounting} onSelectedSevice={(value) => setDtrMounting(value)} />
      ...
      <View style={GeneralAssetDetailStyle.marginTop}>
        ...
        <TextInput
          ...
          style={GeneralAssetDetailStyle.inpView}
          onChangeText={(txt) => setNomberOfFeeder(txt)}
