...
var _codeIndex = ""
// create a component
const EditOtherGeotagScreen = () => {
    let item = null;
    let watchID = null;
    const mapRef = React.useRef(null);
    const route = useRoute();
    const navigation = useNavigation();
    const { oldItem, selectedDistricts, parentData, isRecentGeotagList } = route.params;
    ...
    const onSetOldData = () => {
        ...
        setPackage(oldItem?.package)
        setImageUploadUrl(oldItem?.images);
        ...
        var data = {
            ...
            block: oldItem?.block,
            village: oldItem?.village,
            address: oldItem?.address,
            currentdate: moment(oldItem?.created_at),
        };
        ...
        setTimeout(() => {
            mapRef.current?.animateToRegion({
                latitude: parseFloat(oldItem?.latitude),
                longitude: parseFloat(oldItem?.longitude),
                latitudeDelta: 0.005,
    ...
    function onSubmit() {
        navigation.navigate("EditOtherGeotagFormScreen", { selectedDistricts, parentData, imagesUrl, locationDetail: isLocationDetail, otherPackage: isPackage, oldItem })
    }
            ...
            <KeyboardAwareScrollView>
                <ScrollView>
                    ...
                    {/* <View style={{ marginTop: 10, paddingHorizontal: 10 }}>
                        <SingleDropdownList data={list} defoultTitle={_codeIndex} onSelectedSevice={(value) => { onSelectValue(value) }} />
                    </View> */}

                    <View style={{ marginTop: 10, paddingHorizontal: 10 }}>
                        <DropdownList title={"Package No."} defoutItem={oldItem?.package} data={parentData?.package_data} onSelectedSevice={(value) => { setPackage(value) }} />
                    ...
                    <View style={{ paddingHorizontal: 10 }}>
                        ...
                        <View style={PageStyle.mapConstainer}>
                            ...
                            <MapView style={{ width: '100%', height: '100%' }}
                                ...
                                {isRecentGeotagList && isRecentGeotagList.map((data, index) => (
                                    <Marker
                                        key={index}
                        ...
                        {imagesUrl.length > 0 && (
                            <>
                                <GeotagThreeImageOptimazeModule type="DTR" item={imagesUrl} handlePressImage={(index) => null} removeImage={(index) => onRemoveImage(index)} />

                                {isLocationDetail?.latitude && <AddressCard data={
                                    {
                                        title: oldItem?.title,
                                        subTitle: oldItem?.index_id,
...
export default memo(EditOtherGeotagScreen);
